...
export const HomePage: React.FC<HomePageProps> = ({
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-150 pb-10">
      {/* Dynamic Shop Hero Section */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 text-white p-6 sm:p-8 shadow-xl border border-slate-800">
        {/* Background Ambient Glow */}
        <div className="absolute top-0 right-0 -mt-8 -mr-8 w-72 h-72 bg-blue-500/15 rounded-full blur-3xl pointer-events-none" />
        <div className="absolute bottom-0 left-1/3 -mb-8 w-60 h-60 bg-emerald-500/10 rounded-full blur-2xl pointer-events-none" />

        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="space-y-2 max-w-2xl">
            {/* Shop Badge & Role */}
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-blue-600/30 border border-blue-400/40 text-blue-300 text-[11px] font-bold tracking-wide">
                <Store className="w-3.5 h-3.5" />
                <span>কম্পিউটার ও ডিজিটাল স্টুডিও পোর্টাল</span>
              </span>
              ...
              {isLoggedIn ? (
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-emerald-500/20 border border-emerald-400/40 text-emerald-300 text-[11px] font-bold">
                  ...
                  <span>{isSuperAdmin ? '👑 সুপার এডমিন' : `🏪 ${currentProfile.ownerName}`}</span>
                </span>
              ) : (
                <button
                  onClick={() => onOpenLoginModal('shop_login')}
                  className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-amber-500/20 hover:bg-amber-500/30 border border-amber-400/40 text-amber-300 text-[11px] font-bold transition"
                >
                  <Sparkles className="w-3.5 h-3.5" />
                  <span>দোকানের নাম পরিবর্তন করতে লগইন করুন ↗</span>
                </button>
              )}
            </div>

            {/* Main Shop Name Display */}
            ...
            <p className="text-xs sm:text-sm text-slate-300 font-medium leading-relaxed">
              {currentProfile.tagline || 'অনলাইন সেবা, ফটোকপি, ভোটার তথ্য যাচাই ও ডিজিটাল ফটো স্টুডিও'}
            </p>

            {/* Contact Details Pill */}
            {(currentProfile.phone || currentProfile.address) && (
          ...
          <div className="flex flex-wrap md:flex-col items-stretch gap-2.5 shrink-0">
            <button
              onClick={() => onOpenLoginModal(isLoggedIn ? 'profile' : 'shop_login')}
              className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 hover:bg-white/15 border border-white/20 text-white font-bold text-xs shadow-sm transition backdrop-blur-xs"
            >
              <Edit3 className="w-4 h-4 text-amber-400" />
              <span>{isLoggedIn ? 'দোকানের নাম ও তথ্য এডিট' : 'দোকান লগইন / নাম পরিবর্তন'}</span>
            </button>

            <button
              onClick={() => onSelectTool('voter_search')}
              className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs shadow-md transition"
            >
              <Search className="w-4 h-4" />
              <span>ভোটার ও NID সার্চ শুরু করুন</span>
            </button>
          </div>
        </div>
        ...
        <div className="mt-6 pt-5 border-t border-slate-800/80 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          <div className="flex items-center gap-2 text-slate-300">
            ...
            <span>৩০০ DPI আল্ট্রা-শার্প প্রিন্ট</span>
          </div>
          <div className="flex items-center gap-2 text-slate-300">
            <div className="w-2 h-2 rounded-full bg-blue-400"></div>
            <span>ইনস্ট্যান্ট NID ক্রপ ও জয়েন</span>
          </div>
          <div className="flex items-center gap-2 text-slate-300">
            <div className="w-2 h-2 rounded-full bg-sky-400"></div>
            <span>পাসপোর্ট AI কালার রিমুভ</span>
          </div>
          <div className="flex items-center gap-2 text-slate-300">
            <div className="w-2 h-2 rounded-full bg-purple-400"></div>
            <span>৪R ও A4 প্যাকেজ শিট</span>
          </div>
        </div>
      </div>

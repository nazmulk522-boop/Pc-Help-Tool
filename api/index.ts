...
// Background Removal API (Remove.bg integration)
app.post('/api/remove-bg', async (req, res) => {
  try {
      ...
      try {
        ...
        if (fetchRes.ok) {
          ...
          return res.status(fetchRes.status).json({
            success: false,
            error: `Remove.bg API Error: ${errData}`,
    ...
    return res.json({
      success: false,
      useClientEngine: true,
      message: 'REMOVE_BG_API_KEY পাওয়া যায়নি। অনুগ্রহ করে Vercel Environment Variables অথবা সেটিংস থেকে কী (Key) যুক্ত করুন।',
    });
  } catch (error: any) {
...
// Dedicated Gemini Passport Photo Enhancement
app.post('/api/gemini/enhance-passport', async (req, res) => {
  try {
    ...
    if (!ai) {
      return res.status(400).json({ error: 'GEMINI_API_KEY পাওয়া যায়নি। Vercel Environment Variables চেক করুন।' });
    }
...
2. "primarySubjectBox": {"ymin": number, "xmin": number, "ymax": number, "xmax": number}
3. "recommendedBgColor": string (e.g. "#4A90E2" or "#FFFFFF" or "#C8DCF0")
4. "faceBox": {"ymin": number, "xmin": number, "ymax": number, "xmax": number}
5. "enhancementAdvice": string (short advice in Bengali)`,
          },
        ],
      },
      config: {
...
// Quick Document / Bio-data AI Assistant
